...
function Navbar({ onMenuClick, visitorName, onChangeNameRequest }) {
  ...
  return (
    <header className="relative z-[9999] flex h-16 items-center justify-between border-b border-slate-200/80 bg-white/90 px-4 md:px-8 backdrop-blur-xl dark:border-slate-800/80 dark:bg-[#08120c]/90 transition-colors">
      ...
      <div className="flex flex-1 items-center gap-3">
        ...
        <div className="relative w-full max-w-md" ref={searchRef}>
          ...
          {showSearchDropdown && searchResults.length > 0 && (
            <div className="absolute left-0 right-0 mt-2 min-w-[280px] sm:min-w-[340px] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900 z-[9999] animate-in fade-in duration-150">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-teal-700 dark:bg-slate-800 dark:text-teal-400">
                          <IconComponent size={18} />
                        </div>
                        <div className="min-w-0 flex-1">
                          <h4 className="text-xs sm:text-sm font-extrabold text-slate-900 dark:text-white leading-snug whitespace-normal">
                            {item.title}
                          </h4>
                          {item.subtitle && (
                      ...
                      {/* CATEGORY TAG BADGE (Hidden on mobile to give 100% full width to place names) */}
                      <span className="hidden sm:inline-block rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300 shrink-0 ml-2">
                        {item.category}
                      </span>
                    </div>
                  );
      ...
      <div className="ml-4 flex shrink-0 items-center gap-3 md:gap-4">
        ...
        <div className="relative" ref={notifRef}>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-3 w-80 sm:w-96 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900 z-[9999] animate-in fade-in slide-in-from-top-2 duration-200">
              {/* Header */}
